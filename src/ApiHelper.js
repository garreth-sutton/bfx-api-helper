...
class ApiHelper {
     ...
     */
    constructor(credentialsPath = "credentials.json", suppressWarnings = false, defaultOptionsPath = "defaults.json") {
        try {
    ...
    setContext(
        ...
        return new Promise((resolve, reject) => {
            if (!path) return reject("Path is not set, unable to use the API without specifying an API path.")
            if (!body) body = {}
    ...
    openSocket(options = this.options, delayResponse = 0, nonce = null) {
        ...
        return new Promise((resolve, reject) => {
            ...
            if (!this.key || !this.secret)
                return reject("Unable to create WebSocket connection; credentials were not supplied.")

            let socket = new WebSocket(options.baseWsUrl)

            const authenticationPayload = this.getWebSocketSignature()
            if (options.dms)
                authenticationPayload.dms = 4 // 4 = DMS enabled
            ...
            socket.onmessage = (msg) => {
                ...
                if (response.event == "auth") {
                    ...
                    } else if (response.status == "FAILED" && response.msg == "rate: limit") {
                        console.log("Socket connection refused: Rate limit reached - retrying in 5 seconds")
                        setTimeout(() => openSocket(callback, shouldDelayCallback), 5000)
                    } else {
                        return reject(`Socket authentication failed: ${JSON.stringify(response, null, 2)}`)
                    }
                }
            }
        })
    ...
    sendPostRequest(authenticatedRequest = true, nonce = null) {
        return new Promise((resolve, reject) => {
            this.setNonce(nonce)
            this.sendRequest(true, authenticatedRequest, () => this.error ? reject(this) : resolve(this))
        })
    }
    ...
    sendGetRequest(authenticatedRequest = false, nonce = null) {
        return new Promise((resolve, reject) => {
            this.setNonce(nonce)
            this.sendRequest(false, authenticatedRequest, () => this.error ? reject(this) : resolve(this))
        })
    }
    ...
    actOnResponse(actionFunction) {
        return new Promise((resolve, reject) => {
            actionFunction(this.response, { requestBody: this.body, requestPath: this.getFullPath() })
            return resolve(this)
        })
    }
    ...
    // Private Setters 
    setCredentials(key = null, secret = null) {
        if (key && secret) {
            this.key = key
            this.secret = secret
        }
    ...
    getWebSocketSignature() {
        ...
        const authSig = CryptoJS.HmacSHA384('AUTH' + this.nonce, this.secret).toString(CryptoJS.enc.Hex)
    ...
    sendRequest(isPost, authenticatedRequest, callback) {
        ...
        if (authenticatedRequest && (!this.key || !this.secret)) {
            console.log(
        ...
        let requestPayload = this.version === 1 ? this.getRequestPayloadV1(authenticatedRequest) : this.getRequestPayloadV2(authenticatedRequest)

        if (this.isVerbose)
            console.log(`Verbose - Request Payload: ${JSON.stringify(requestPayload)}`)
        ...
        executeRequest(requestPayload, (error, response, body) => {
            ...
            if (this.isVerbose)
                console.log(`Verbose - Response Body: ${this.version === 1 ? body : JSON.stringify(body)}`)
            callback()
        })
    }
}
